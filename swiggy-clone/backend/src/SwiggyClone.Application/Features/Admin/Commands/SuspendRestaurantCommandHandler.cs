...
internal sealed class SuspendRestaurantCommandHandler(IAppDbContext db, IPublisher publisher)
    : IRequestHandler<SuspendRestaurantCommand, Result<AdminRestaurantDto>>
{
    public async Task<Result<AdminRestaurantDto>> Handle(SuspendRestaurantCommand request, CancellationToken ct)
    {
        ...
        if (restaurant.Status != RestaurantStatus.Approved)
            return Result<AdminRestaurantDto>.Failure("INVALID_STATUS", "Only approved restaurants can be suspended.");

        restaurant.Status = RestaurantStatus.Suspended;
        restaurant.StatusReason = request.Reason;
        await db.SaveChangesAsync(ct);

        await publisher.Publish(new RestaurantDeleteFromIndexRequested(restaurant.Id), ct);
