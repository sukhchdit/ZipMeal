...
internal sealed class UpdateGroupCartItemCommandHandler(
    IAppDbContext db,
    IGroupCartService groupCartService,
    IPublisher publisher)
    : IRequestHandler<UpdateGroupCartItemCommand, Result<CartDto>>
{
    public async Task<Result<CartDto>> Handle(UpdateGroupCartItemCommand request, CancellationToken ct)
    {
        ...
        var result = await groupCartService.UpdateQuantityAsync(
            request.GroupOrderId, request.UserId, request.CartItemId, request.Quantity, ct);
