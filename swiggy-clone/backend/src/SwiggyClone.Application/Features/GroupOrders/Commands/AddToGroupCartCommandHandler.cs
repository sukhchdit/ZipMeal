...
internal sealed class AddToGroupCartCommandHandler(
    IAppDbContext db,
    IGroupCartService groupCartService,
    IPublisher publisher)
    : IRequestHandler<AddToGroupCartCommand, Result<CartDto>>
{
    public async Task<Result<CartDto>> Handle(AddToGroupCartCommand request, CancellationToken ct)
    {
        ...
        if (groupOrder.ExpiresAt <= DateTimeOffset.UtcNow)
            return Result<CartDto>.Failure("GROUP_ORDER_EXPIRED", "This group order has expired.");
        ...
        var result = await groupCartService.AddToCartAsync(request.GroupOrderId, request.UserId, item, ct);
