...
internal sealed class RejectRestaurantCommandHandler(IAppDbContext db, IPublisher publisher)
    : IRequestHandler<RejectRestaurantCommand, Result<AdminRestaurantDto>>
{
    public async Task<Result<AdminRestaurantDto>> Handle(RejectRestaurantCommand request, CancellationToken ct)
    {
        ...
        if (restaurant.Status != RestaurantStatus.Pending)
            return Result<AdminRestaurantDto>.Failure("INVALID_STATUS", "Only pending restaurants can be rejected.");
